...
class Renderer:
    def __init__(self, intervals, footer_list) -> None:
        ...
        interval_x, interval_y = self.get_coordinates_for_centered_text(
            " " * (len(str(intervals)) * 2 + 2)
        )
        self.windows = [
            self.screen,  # 0: main_scr
            ...
            newwin(1, len(str(intervals)) * 2 + 2, interval_y + 1, interval_x + 1),
        ]  # 4: interval x/N
    ...
    def init_footer(self, args: list, max_len: int) -> None:
        ...
        if char_amount // 2 < max_len:
            ...
            lists = [
                "".join([arg + " " * width_1 for arg in lists[0]]).strip(),
                "".join([arg + " " * width_2 for arg in lists[1]]).strip(),
            ]
            if self.windows[1].getbegyx()[0] >= self.windows[4].getbegyx()[0]:
...
def main(args):
    ...
    def run_prog():
        ...
        for i in range(args.intervals * 2 - 1):
            if i % 2 == 0:
                current_interval += 1
            screen.output_text_to_window(
                4,
                f"{current_interval:0{len(str(args.intervals))}}/{args.intervals}",
                0,
                0,
            )
            screen.output_text_to_window(
                3, title[break_timer_enabled], 0, 0, A_UNDERLINE
            )
            ...
            while not current_timer.is_finished():
                sleep(0.1)
                if (key := screen.get_input()) is not None:
                    key = key.upper()
                match key:
                    case "Q":
                        screen.kill_scr()
                    ...
                    case "R":
                        ...
                        timer[1].reset()
                        screen.output_text_to_window(
                            4, f"{1:0{len(str(args.intervals))}}/{args.intervals}", 0, 0
                        )
                        screen.output_text_to_window(3, title[0], 0, 0, A_UNDERLINE)
                        screen.output_text_to_window(
                            2, timer[0].__str__(), 0, 0, A_STANDOUT
                        )
                        sleep(3)
                        run_prog()
                    case "U" | "KEY_RESIZE":
                        ...
                        screen.output_text_to_window(
                            2, current_timer.__str__(), 0, 0, A_STANDOUT
                        )
            ...
            try:
                ...
                logging.error(
                    f"Something went wrong with playing the {file} file. Make shure it exists in the folder of this python file.\n{split(argv[0])[0]}"
                )
                sleep(3)
